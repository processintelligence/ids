...
# PUBLIC API
def generate_config_structure(pnml_path: str, config_dir: str) -> str:
    """
    Public function that replaces PNMLToDataPNML.generate_config_structure().
    Creates:
      - A new PNML with an artificial init place/transition (data_*.pnml in GeneratedFiles/DataPNML)
      - A blank config JSON in config_dir
...
def generate_data_petrinet(config_path: str) -> str:
    """
    Generate a data-annotated PNML based on an existing config file.
    
    Args:
        config_path: Full path to the config JSON file.

    Returns:
        Path to the generated data PNML file.
    """
    # Validate config
    ...
    # Determine where the PNML lives
    ...
    # Prefer wrapped PNML if available
...
def _create_pnml(petri_net, im, fm, output_folder: str) -> str:
...
def _create_blanc_config(petrinet, pnml_path: str, config_dir: str) -> str:
    ...
    # Name based on original PNML file
...
def _create_data_pnml(guards, data_pnml_path: str, config_path: str):
